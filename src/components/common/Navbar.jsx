...
return(
    <div className={`flex sm:relative bg-[#000814] w-screen relative z-50 h-14 items-center justify-center border-b-[1px] border-b-[#2C333F] translate-y-3 transition-all duration-500`}>
        <div className='flex w-14/12 max-w-["1260px"] items-center justify-between'>
            <Link to='/' onClick={() => { dispatch(setProgress(100)) }}>
                ...
                <div className={`flex md:hidden relative flex-row ${token !== null && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR ? "" : ""}`}>
                    <GiHamburgerMenu className={`cursor-pointer fill-[#DBDDEA] absolute -bottom-4`} onClick={shownav}/>
                    <div ref={overlay} className='fixed w-screen h-screen z-30 hidden bg-[rgba(0,0,0,0.5)]' onClick={shownav}></div>
                    <div ref={show} className='mobNav z-50 bg-[rgba(0,0,0,0.5)]'>
                        <nav className='items-center flex flex-col absolute w-[200px] -left-[0px] -top-7 glass2'>
                            {
                                token == null && (
                ...
                <nav>
                    <ul className='flex-row gap-x-6 text-[#DBDDEA] gap-5 hidden md:flex'>
                                        ...
                                        element.title === "Catalog" ? (<div className=' flex items-center group relative cursor-pointer'>
                                            ...
                                            <div className='invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-["#F1F2FF"] p-4 text-["#000814"] opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]'>
                                                <div className='absolute left-[50%] top-0 -z-10 h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded bg-[#F1F2FF]'></div>
                                                {
                                                    sublinks?.length < 0 ? (<div></div>) : (
                                                        sublinks?.map((element, index) => (
                                                            <Link to={`/catalog/${element?.name}`} key={index} className="rounded-lg bg-transparent py-4 pl-4 hover:bg-[#C5C7D4]" onClick={() => { dispatch(setProgress(30)) }}>
                                                                <p className=''>
                                                                    {element?.name}
                                                                </p>
                                                            </Link>
                        ...
                        <form onSubmit={handelSearch} className='flex items-center relative'>
                            <input value={searchValue} onChange={(e) => { setSearchValue(e.target.value) }} id='searchinput' type="text" placeholder="Search" className=' absolute top-0 left-0 border-0 focus:ring-1 ring-[#6E727F] rounded-full px-2 py-1 text-[15px] w-28 text-[#C5C7D4] focus:outline-none focus:border-transparent bg-[#2C333F]' />
                ...
                <div className='flex-row gap-5 hidden md:flex items-center'>
                    {
                        user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
                            <Link to='/dashboard/cart' className='relative px-4' onClick={() => { dispatch(setProgress(100)) }} >
                                ...
                                {
                                    totalItems > 0 && (
                                        <span className='shadow-sm shadow-black text-[10px] font-bold bg-yellow-500 text-[#000814] rounded-full px-1 absolute -top-[2px] right-[8px]'>
                                            {totalItems}
                                        </span>
                                    )
                                }
