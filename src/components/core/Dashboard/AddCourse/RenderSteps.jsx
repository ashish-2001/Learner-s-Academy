                    ...
                    <>
                        <div className="flex flex-col items-center" key={item.id}>
                            <button className={`grid cursor-default aspect-square w-[34px] item-center rounded-full border-[1px] ${
                                step === item.id? "border-yellow-50 bg-yellow-900 text-yellow-600"
                                : "border-white bg-blue-950 text-white"
                            } ${step > item.id && "bg-yellow-600 text-yellow-50"}`}>
                                {step > item.id ? (
                        ...
                        {item.id !== steps.length && (
                            <>
                                <div className={`h-[calc(34px/2)] w-[33%] border-dashed border-b-2 ${
                                    step > item.id ? "border-yellow-600" : "border-white"
                                }`}></div>
                            </>
                        )}
                    </>
                ...
                {steps.map((item) => {
                    <>
                        <div className="flex min-w-[130px] flex-col items-center gap-y-2" key={item.id}>
                            <p className={`text-sm ${
                                step >= item.id ? "text-white" : "text-white"
                            }`}>
                                {item.title}
                            </p>
                        </div>
